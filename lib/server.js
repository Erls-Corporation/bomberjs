...
var Server = exports.Server = function(app) {
  ...
  if (!this.options.security.signing_secret) {
    ...
    this.options.security.signing_secret = require('./sha1').hex_hmac_sha1(Math.random(), Math.random());
  }

  // Prepare persistent storage
  this.store = new DirectoryStore(this.options.persistent_storage.options.location);
...
Server.prototype.start = function() {
  ...
  http.createServer(function (req, res) {
      ...
      }).listen(server.options.server.port);
