...
var SessionStore = require('./session_store').SessionStore;
...
var Server = exports.Server = function(app) {
  ...
  var defaults = require('./config_defaults').defaults;
  this.base.config = this.base.config || {};
  this.options = {};
  for (setting in defaults) {
    this.options[setting] = this.base.config[setting] || defaults[setting];
  }

  // Warn if no signing secret is set -- and create a temporary one
  if (!this.options.signing_secret) {
    sys.error(
    ...
    this.options.signing_secret = require('./sha1').hex_hmac_sha1(Math.random(), Math.random());
  }

  // Prepare persistent storage
  this.store = new DirectoryStore(this.options.persistent_storage_location);
  this.session_store = new SessionStore(this);
};
...
Server.prototype.start = function() {
  ...
  http.createServer(function (req, res) {
      try {
        ...
        request.session = response.session = server.session_store.getCurrentSession(request, server);
        ...
        // Save the session (this is unlikely to be the perfect place for it, at last if we're running stuff async)
