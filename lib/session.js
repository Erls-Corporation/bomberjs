...
Session.prototype.reset = function() {
  ...
  this._data = {
    __created: (+new Date),
    __renew: (+new Date( +new Date + (this._server.options.session_renew_minutes*60*1000) )),
    __expires: (+new Date( +new Date + (this._server.options.session_expire_minutes*60*1000) ))
  }
}
...
Session.prototype.possiblyRenewSessionCookie = function() {
  if ( ('__renew' in this._data) && this._data['__renew']<(+new Date()) ) {
    this._data['__renew'] = (+new Date( +new Date + (this._server.options.session_renew_minutes*60*1000) ));
    this._data['__expires'] = (+new Date( +new Date + (this._server.options.session_expire_minutes*60*1000) ));
    this._modified = true;
    this.writeSessionCookie();
  }
}
 ...
 */
Session.prototype.writeSessionCookie = function() {
  var opts = this._server.options;
  var cookie_opts = {
    expires: new Date(this._data['__expires']),
    domain: opts.session_cookie_domain,
    path: opts.session_cookie_path,
    secure: opts.session_cookie_secure
  };
  this._request.cookies.setSecure( opts.session_cookie_name, this.session_key, cookie_opts );
}
